...
const main = async () => {
  ...
  if (cmd === 'deploy') {
    ...
    if (!which || !['task', 'proof', 'matching'].includes(which)) usage();
    ...
    try {
      if (which === 'task') {
        ...
        const privateState: TaskPrivateState = {
        ...
        const privateState: ProofPrivateState = {
        ...
        const privateState: MatchingPrivateState = {
          taskSecret: randomBytes(32),
      ...
      if ('stop' in wallet && typeof (wallet as any).stop === 'function') {
